    ...
    render() {
        ...
        <h3 className="languageHeadline">{this.props.german ? <>Bitte wähle einen Stil</> : <> Please choose your favorite style</>}</h3>
                <div className="btnLine">
                    <button className ="colorBtn" onClick={()=>{this.props.switchStyle("linkColor1", "headerColor1", "navBgTrans")}}>{this.props.german ? <>Stil 1</> : <>style 1</>}</button>
                    <button className ="colorBtn" onClick={()=>{this.props.switchStyle("linkColor2", "headerColor2", "navBg2")}}>{this.props.german ? <>Stil 2</> : <>style 2</>}</button>
